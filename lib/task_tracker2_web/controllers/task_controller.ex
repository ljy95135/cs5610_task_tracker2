defmodule TaskTracker2Web.TaskController do
  ...
  def create_with_login(conn, %{"to_id" => to_id, "title" => title, "body" => body}) do
    from_id = get_session(conn, :user_id)
    task = %{from_id: from_id, to_id: to_id, title: title, body: body, isFinished: false}

    case Work.create_task_login(task) do
      {:ok, task} ->
        conn
      ...
      {:error, %Ecto.Changeset{} = changeset} ->
        conn
        |> put_flash(:error, "Task not created successfully.")
        |> redirect(to: page_path(conn, :feed))
    end
  end
